        ...
        this.setupConfig(yargs.argv.config).then(
            options => {
                ...
                this.saveConfig(options).then(
                    file => {
                        let configParameter = (file != "laravel-echo-server.json" ? ' --config="' + file + '"' : "");

                        console.log(
                            'Configuration file saved. Run ' + colors.magenta.bold('laravel-echo-server start' + configParameter) + ' to run server.'
                        );
                ...
                if (lockProcess) {
                    try {
                        ...
                        if (yargs.argv.force) {
                            ...
                            console.log(
                                colors.yellow("Warning: Closing process " + lockProcess + " because you used the '--force' option.")
                            );
                        } else {
                            console.error(
                                colors.error("Error: There is already a server running! Use the option '--force' to stop it and start another one.")
                            );
